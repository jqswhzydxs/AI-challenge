...
算法组 2026-07-24 最新说明：数据预处理部分的插值方法已从线性插值更新为 PCHIP（分段三次埃尔米特插值）。新版分钟级数据基于连续 7 天数据重采样生成，共 3,155,026 条。相比线性插值或普通三次样条，PCHIP 更适合工业负荷曲线，可保持局部单调性并减少过冲。

该变更仅影响算法预处理与训练/拟合数据来源口径。算法最终输出给后端的 JSON 格式、字段定义及单位保持不变，因此当前数据库字段和后端接口无需因 PCHIP 调整而变更。
...
| `EC_baseline` | `ec_baseline` | 优化前 EC 基准值，kWh/吨 |
| `EC_optimized` | `ec_optimized` / `elec_coefficient` | 优化后 EC 值，kWh/吨；后端同时作为小时电耗推导系数 |
| `EC_reduction` | `ec_reduction` | EC 降低百分比 |
| `optimal_temperature` | `optimal_temperature` | 最优温度，℃ |
| `optimal_speed` | `optimal_speed` | 最优速度 |
| `total_production` | `total_production` | 24 小时总排产量，吨 |
...
| `schedule[].elec_forecast` | `elec_forecast` | 旧版字段；新版 JSON 已移除时，后端按 `production × EC_optimized` 推导 |

后端建议将完整 `daily_plan.json` 原文保存在 `algorithm_task.algorithm_response_json`，同时拆分写入 `production_schedule_plan`、`production_schedule_detail` 和 `evaluation_metric`。PCHIP 更新不改变 `daily_plan` JSON 字段；新版算法技术报告新增 MAPE 评价指标，后端可保存到 `evaluation_metric.mape`，当前 JSON 样例未携带时允许为空。
...
| `training_record_count` | int | 否 | 模型训练/拟合使用记录数，如新版 PCHIP 分钟级数据 `3155026` |
...
| `source` | varchar(128) | 否 | 数据来源，默认 `steel_data_cleaned` |

索引建议：

- 唯一索引：`uk_timestamp_source(timestamp, source)`。15 分钟数据和 1 分钟数据会共用 2018 年时间戳，唯一键必须包含来源。
...
| `data_quality` | varchar(32) | 否 | - | 数据质量/导入批次标识。当前导入脚本用 `steel_data_cleaned`、`steel_data_1min_fixed` 区分 15 分钟与 1 分钟数据。 |

索引建议：

- 唯一索引：`uk_dataset_timestamp_source(timestamp, data_quality)`。当前表没有独立 `source` 字段，因此用 `data_quality` 承载导入批次标识，避免 15 分钟与 1 分钟数据时间戳冲突。
- 普通索引：`idx_load_type(load_type)`
- 普通索引：`idx_nsm(nsm)`
...
用于保存 MAPE、EC、ER 和优化前后对比指标。算法组新版技术报告已新增 MAPE 指标，当前示例值为 `3.55%`，对应仿真精度 `96.45%`。
...
- MAPE：新版算法技术报告已给出口径和结果，后端保存到 `evaluation_metric.mape`；当前 `daily_plan` JSON 字段契约不因 PCHIP 更新而变化，未携带该指标时允许为空或使用演示数据。
- EC：`daily_plan` 已提供 `EC_baseline`、`EC_optimized`、`EC_reduction`，后端映射到优化前后 EC 指标。
- ER：当前仍由后端根据方案状态、冲突记录或演示口径派生，后续如算法补充则按算法输出入库。
...
| 时间粒度是否统一采用 15 分钟 | 原始能源数据为 15 分钟/点；算法预处理已通过 PCHIP 重采样到 1 分钟粒度；当前 `daily_plan` JSON 仍按小时汇总 |
| 是否输出 MAPE/EC/ER | MAPE 和 EC 当前已由算法侧提供，ER 暂由后端派生；影响 `evaluation_metric` 和后端计算逻辑 |
...
如旧唯一键已经删除或新唯一键已经存在，对应报错可忽略。

导入脚本必须按下面顺序执行：

1. `seed_demo_data.sql`：基础演示数据，幂等。
2. `seed_steel_data.sql`：清空 `energy_realtime_data` 和 `steel_energy_dataset`，导入 35,040 行 15 分钟数据，来源标识 `steel_data_cleaned`。
3. `seed_steel_1min.sql`：追加 1 分钟数据，来源标识 `steel_data_1min_fixed`。算法组最新正式口径为 PCHIP 重采样后 3,155,026 行；若当前仓库仍是旧联调样本 `132,481` 行，需等新版 CSV 到位后重新生成该 seed。
4. `seed_mpc_control.sql`：清空 MPC 结果并导入 120 条 final，日期 `2026-07-17`。
5. `seed_mpc_hourly.sql`：追加 120 条 hourly，日期 `2026-07-18`。

注意：`seed_steel_data.sql` 会清空能源实时表和钢铁样本表，所以必须排在第 2 步。若最终还需要保留 `seed_demo_data.sql` 中 48 条 2026 年演示实时数据，可在第 2 步后幂等重跑一次 `seed_demo_data.sql` 补回。MySQL 安全模式下不要使用无 WHERE 的手工 DELETE；如果导入中断后只需清理 1 分钟数据，执行：

```sql
DELETE FROM energy_realtime_data WHERE source = 'steel_data_1min_fixed';
```
