...
本系统面向工业园区轧钢生产与能源供给协同优化场景，围绕“日级生产排产方案 + 实时能源运行方案”的一体化生成进行设计。系统通过前端上传历史能源数据 CSV，并填写计划日期、当天总需求吨数和最晚完成时间，由后端创建算法任务，调用 Python 工程化算法生成小时级生产排产方案和 MPC 实时调控结果，再将结果入库并展示给前端。

当前系统不是订单级 APS 排产系统。算法输入的生产需求是日总产量 `D_total`，不包含订单编号、单订单重量、交付窗口和订单优先级等订单级字段。因此系统规划对象是：

```text
在给定日总产量 D_total 的前提下，
规划 24 小时内每小时生产多少，
并生成与该生产计划匹配的能源调控方案。
```
...
- 支持用户登录、角色识别和基础权限控制。
- 支持上传历史能源 CSV，并接收计划日期、总需求吨数、最晚完成时间等排产输入。
...
```mermaid
flowchart TD
    A["前端上传历史能源 CSV"] --> B["填写计划日期 / D_total / 最晚完成时间"]
    B --> C["后端创建 algorithm_task"]
    C --> D["保存 input.csv 和 config.json"]
    D --> E["调用 Python generate_plan.py"]
    E --> F["算法生成 output.json"]
    F --> G["解析 daily_plan"]
    G --> H["写入 production_schedule_plan/detail"]
...
生产排产模块负责日总量级排产方案生成与展示。当前算法不区分订单，订单列表主要用于业务展示和后续扩展，真实算法输入以 `D_total` 为主。
...
| 接口 | 说明 |
| --- | --- |
| `GET /api/production/orders` | 生产订单列表，当前主要作为展示数据 |
...
生产排产页面应避免使用“订单级排产”表述，建议文案为：

```text
上传历史能源数据，并填写当日生产需求，系统将生成小时级生产-能源协同优化方案。
```
...
| `scheduleDate` | string | 建议必填 | 计划日期，例如 `2026-08-03` |
| `totalDemand` | number | 建议必填 | 当天总需求吨数 |
| `deadline` | string | 建议必填 | 最晚完成时间，例如 `2026-08-03 23:00:00` |
...
当前订单上传接口用于订单展示、需求管理和后续订单级排产扩展；现阶段 Python 算法仍以 `D_total` 作为日总量输入，不直接按订单编号逐单排产。
...
- 将 `scheduleDate`、`totalDemand`、`deadline` 正式接入后端和 Python 算法配置文件。
- 若需要订单级排产，扩展订单列表输入、交货窗口、优先级和工艺路线约束。
