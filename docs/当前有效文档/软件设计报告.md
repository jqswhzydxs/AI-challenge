...
- 支持上传 1 个月以上历史能源 CSV，并接收可选计划日期 `scheduleDate`；15 分钟粒度数据会插值为 1 分钟粒度。
...
flowchart TD
    ...
    G --> I1["解析 daily_plan"]
    I1 --> H["写入 production_schedule_plan/detail"]
    H --> J["更新订单状态为 SCHEDULED"]
    G --> I2["解析 realtime_control"]
    I2 --> J2["写入 mpc_realtime_control"]
    H --> K["派生能源运行方案"]
    K --> L["生成协同评估指标"]
    J --> M["前端展示排产与能源调控结果"]
    J2 --> M
...
| 字段 | 说明 |
| --- | --- |
| `file` | 1 个月以上历史能源 CSV，包含 `timestamp/date` 与 `elec/Usage_kWh`；15 分钟粒度不少于 2880 行，1 分钟粒度不少于 43200 行 |
...
| 数据不足 | 至少需要 1 个月以上连续能源数据；15 分钟粒度不少于 2880 行，1 分钟粒度不少于 43200 行 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `file` | file | 是 | 1 个月以上历史能源 CSV；15 分钟粒度不少于 2880 行，1 分钟粒度不少于 43200 行 |
